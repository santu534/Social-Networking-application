...
public class ChatBlockedListingParser {
	...
	public ChatBlockedListingParser(IItemHandler aHandler, int requestId)
			throws Exception {
		handler = aHandler;
		REQ_TYPE = requestId;
	}
	...
	public void parseXmlData(InputStream inputStream) throws Exception {
		try {
			...
			if(innerObj != null) {
				Item item = new Item("");
				item.setAttribute("total_pages", innerObj.getString("total_pages_block")+"");
				item.setAttribute("perpage", innerObj.getString("perpage")+"");
				item.setAttribute("total", innerObj.getString("total_block_messages")+"");
				item.setAttribute("others_cnt", innerObj.getString("block_cnt")+"");
				items.add(item);
			} 
			
			JSONArray user_videos = (JSONArray) innerObj.get("block");
