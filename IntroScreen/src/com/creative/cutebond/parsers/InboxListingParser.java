...
public class InboxListingParser {
	...
	public InboxListingParser(IItemHandler aHandler, int requestId)
			throws Exception {
		handler = aHandler;
		REQ_TYPE = requestId;
	}
	...
	public void parseXmlData(InputStream inputStream) throws Exception {
		try {
			...
			if(innerObj != null) {
				Item item = new Item("");
				item.setAttribute("total_pages", innerObj.getString("total_pages_inbox")+"");
				item.setAttribute("perpage", innerObj.getString("perpage")+"");
				item.setAttribute("total", innerObj.getString("total_inbox_messages")+"");
				item.setAttribute("inbox_cnt", innerObj.getString("inbox_cnt")+"");
				items.add(item);
			} 
			
			JSONArray user_videos = (JSONArray) innerObj.get("inbox");
