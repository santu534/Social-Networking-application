...
public class MyHubLeftMenuParser {
	...
	public MyHubLeftMenuParser(IItemHandler aHandler, int requestId)
			throws Exception {
		handler = aHandler;
		REQ_TYPE = requestId;
	}
	...
	public void parseXmlData(InputStream inputStream) throws Exception {
		try {
			...
			while (inrItr.hasNext()) {
				...
				Object mainobject = object.get(inrKey);
				
				if(mainobject instanceof JSONArray) {

					JSONArray array = (JSONArray)mainobject;//.getJSONArray(inrKey);
				...
				} else if(mainobject instanceof JSONObject){
					JSONObject jsonobject = (JSONObject)mainobject;
					JSONArray array = (JSONArray)jsonobject.getJSONArray("birthdays");
