...
public class PeoplesListingParser {
	...
	public PeoplesListingParser(IItemHandler aHandler, int requestId)
			throws Exception {
		handler = aHandler;
		REQ_TYPE = requestId;
	}
	...
	public void parseXmlData(InputStream inputStream) throws Exception {
		try {
			...
			if(innerObj != null) {
				...
				item.setAttribute("total_videos", innerObj.getString("total_profiles")+"");
				items.add(item);
			}
			
			JSONArray user_videos = (JSONArray) innerObj.get("profiles");
