...
public class UserContactsListingParser {
	...
	public UserContactsListingParser(IItemHandler aHandler, int requestId)
			throws Exception {
		handler = aHandler;
		REQ_TYPE = requestId;
	}
	...
	public void parseXmlData(InputStream inputStream) throws Exception {
		try {
			...
			JSONArray user_contacts = (JSONArray) innerObj.get("people");

			for (int i = 0; i < user_contacts.length(); i++) {

				item = new Item("values");

				JSONObject obj = user_contacts.getJSONObject(i);
