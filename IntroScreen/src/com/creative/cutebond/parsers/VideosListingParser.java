...
public class VideosListingParser {
	...
	public VideosListingParser(IItemHandler aHandler, int requestId)
			throws Exception {
		handler = aHandler;
		REQ_TYPE = requestId;
	}
	...
	public void parseXmlData(InputStream inputStream) throws Exception {
		try {
			...
			if(innerObj != null) {
				...
				item.setAttribute("total_videos", innerObj.getString("total_videos")+"");
				items.add(item);
			} 
			
			JSONArray user_videos = (JSONArray) innerObj.get("user_videos");
