...
public class YVideosDetailParser {
	...
	public YVideosDetailParser(IItemHandler aHandler, int requestId)
			throws Exception {
		handler = aHandler;
		REQ_TYPE = requestId;
	}
	...
	public void parseXmlData(InputStream inputStream) throws Exception {
		try {
			...
			innerObj = innerObj.getJSONObject("entry");
			
			innerObj = innerObj.getJSONObject("media$group");			
			
			JSONArray user_videos = (JSONArray) innerObj.get("media$content");
